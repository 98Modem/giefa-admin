...
export function ReportEditDialog({
  ...
  const modal =
    open && typeof document !== "undefined"
      ? createPortal(
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-gray-950/55 px-3 py-4 backdrop-blur-md sm:px-6"
          onMouseDown={(event) => {
        ...
        >
          <section
            ...
            className="flex max-h-[92dvh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl ring-1 ring-white/40 dark:border-white/15 dark:bg-gray-950"
          >
            <div className="border-b border-gray-200 bg-white/95 px-4 py-3 backdrop-blur dark:border-white/10 dark:bg-gray-950/95 sm:px-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm leading-5 text-gray-500 dark:text-gray-300">
                    Correct row values and recalculate member allocations.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="min-h-10 min-w-10 rounded-full border border-gray-200 px-3 text-sm font-semibold text-gray-600 transition hover:bg-gray-50 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-300 dark:border-white/15 dark:text-gray-300 dark:hover:bg-white/10 dark:hover:text-white"
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  <label className="grid gap-2 text-sm font-medium text-gray-800 dark:text-gray-100">
                    ...
                    <input
                      ...
                      readOnly
                      className="h-10 rounded-lg border border-gray-200 bg-gray-50 px-3 text-gray-600 outline-none dark:border-white/15 dark:bg-white/5 dark:text-gray-300"
                    />
                  </label>
                  ...
                  <label className="grid gap-2 text-sm font-medium text-gray-800 dark:text-gray-100">
                    ...
                    <input
                      ...
                      className="h-10 rounded-lg border border-gray-200 bg-white px-3 text-gray-900 outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:border-white/15 dark:bg-white/10 dark:text-white dark:focus:ring-brand-400/20"
                    />
                  </label>
                  ...
                  <label className="grid gap-2 text-sm font-medium text-gray-800 dark:text-gray-100 xl:col-span-2">
                    Edit note
                    <textarea
                      name="notes"
                      rows={3}
                  ...
                  <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900 dark:border-amber-300/25 dark:bg-amber-400/10 dark:text-amber-100">
                    Approved deposits are saved as an audited adjustment, then daily weighted interest allocations are recalculated.
                  </div>
                </div>
              </div>

              <div className="border-t border-gray-200 bg-white/95 px-4 py-3 backdrop-blur dark:border-white/10 dark:bg-gray-950/95 sm:px-5">
                <div className="grid gap-3 sm:ml-auto sm:w-fit sm:grid-cols-2">
                  <button
                    type="button"
                    onClick={() => setOpen(false)}
                    className="min-h-11 rounded-lg border border-gray-200 px-6 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-brand-300 dark:border-white/15 dark:text-gray-200 dark:hover:bg-white/10"
                  >
                    Cancel
                  </button>
                  <button
                    disabled={isPending}
                    className="min-h-11 rounded-lg bg-brand-500 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-300 disabled:cursor-not-allowed disabled:opacity-70"
                  >
